...
func ValidateEmail(v *validator.Validator, email string) {
	v.Check(validator.EmailRx.MatchString(email), "email", "must be a valid email")
}

func ValidatePlaintextPassword(v *validator.Validator, plainTextPassword string) {
...
func ValidateUser(v *validator.Validator, user *User) {
	v.Check(len(user.Name) > 0, "name", "must be provided")
...
func (m UserModel) GetForEmail(email string) (*User, error) {
	query := `
...
func (m UserModel) GetForToken(plainTextToken string, tokenScope string) (*User, error) {
	query := `
